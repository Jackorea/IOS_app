...
struct BatchDataStatsView: View {
        ...
        VStack(spacing: 16) {
            ...
            if !bluetoothViewModel.isConnected {
                ...
                LazyVStack(spacing: 12) {
                    ...
                    if bluetoothViewModel.latestPPGReading != nil {
                        sensorStatCard(for: "PPG", icon: "heart.fill", color: .red)
                    }
                    
                    if bluetoothViewModel.latestAccelerometerReading != nil {
                        sensorStatCard(for: "가속도계", icon: "move.3d", color: .blue)
                    }
                    
                    if bluetoothViewModel.latestBatteryReading != nil {
            ...
            HStack {
                ...
                VStack(alignment: .trailing, spacing: 2) {
                    Text("기록 상태")
                        .font(.caption2)
                        .foregroundColor(.secondary)
                    Text(bluetoothViewModel.isRecording ? "기록 중" : "대기")
                        .font(.caption)
                        .fontWeight(.medium)
                        .foregroundColor(bluetoothViewModel.isRecording ? .red : .gray)
                }
            }
        }
        .padding()
    ...
    private func sensorDataView(for sensorType: String, color: Color) -> some View {
        switch sensorType {
        case "EEG":
            if let eeg = bluetoothViewModel.latestEEGReading {
        ...
        case "PPG":
            if let ppg = bluetoothViewModel.latestPPGReading {
                LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 2), spacing: 8) {
        ...
        case "가속도계":
            if let accel = bluetoothViewModel.latestAccelerometerReading {
        ...
        case "배터리":
            if let battery = bluetoothViewModel.latestBatteryReading {
...
#Preview {
    BatchDataStatsView(bluetoothViewModel: BluetoothKitViewModel())
} 